...
// ── tiny card extracted so each has its own refs ──────────────────────────────
function ProjectCard({ p, i }: { p: any; i: number }) {
    ...
    return (
        ...
        >
            <div
                className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] backdrop-blur-xl transition-all duration-300 hover:border-[#6366f1]/40 hover:shadow-lg"
            ...
            >
                ...
                <div className="relative aspect-[16/10] overflow-hidden bg-[var(--bg-secondary)]">
                    <Image
                        ...
                        alt={p.title}
                        fill
                        className={`object-cover transition-opacity duration-500 ${hovered && p.video ? 'opacity-0' : 'opacity-100'}`}
                        unoptimized
                    />
                    {p.video && (
                        <video
